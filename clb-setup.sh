...
FWD_RULE="solis-dis-forwarding-rule"
...
echo ""
echo ">>> Step 7: Creating HTTP(S) target proxy '${HTTPS_PROXY}'..."

if gcloud compute target-http-proxies describe "${HTTPS_PROXY}" --project="${PROJECT_ID}" &>/dev/null; then
    echo "    Proxy '${HTTPS_PROXY}' already exists — skipping."
else
    # Using HTTP proxy for initial testing (no SSL cert needed)
    # For production, switch to target-https-proxies with a managed SSL cert
    gcloud compute target-http-proxies create "${HTTPS_PROXY}" \
        --url-map="${URL_MAP}" \
        --global \
        --project="${PROJECT_ID}"
    echo "    HTTP proxy created."
    echo "    NOTE: Using HTTP for testing. For production, create an SSL cert and use target-https-proxies."
fi

# ─── Step 8: Create Forwarding Rule ───
echo ""
echo ">>> Step 8: Creating forwarding rule '${FWD_RULE}'..."
...
echo ""
echo " Load Balancer IP : ${LB_IP}"
echo " Test URL (DIS)   : http://${LB_IP}/api/dis/bc/publish"
echo " Test URL (ADJ)   : http://${LB_IP}/publish"
echo ""
echo " API Key source: Secret Manager (${SECRET_NAME})"
echo ""
echo " Example curl test:"
echo "   curl -X POST http://${LB_IP}/api/dis/bc/publish \\"
echo "     -H 'Content-Type: application/json' \\"
echo "     -H 'x-api-key: \$(gcloud secrets versions access latest --secret=${SECRET_NAME} --project=${PROJECT_ID})' \\"
echo "     -d '{\"test\": true}'"
echo ""
echo " Next steps:"
echo "   1. Update application-cloud.properties GATEWAY_HOST to: ${LB_IP}"
echo "   2. API key is auto-loaded from Secret Manager via sm:// — no env var needed"
echo "   3. Lock down Cloud Run services:"
